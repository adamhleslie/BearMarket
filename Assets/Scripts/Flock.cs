...
public class Flock : MonoBehaviour
{
  ...
  void FixedUpdate ()
  {
    ...
    for (int activeBoid = boidList.Count - 1; activeBoid >= lastActiveBoid; activeBoid--)
    {
      ...
      {
        RaycastHit hit;
        if (Physics.SphereCast(boid.body.position, 5f, boid.body.velocity,
              out hit, wallDetectDist, collisionLayer))
        {
          Vector3 reflectDir = Vector3.Reflect(boid.body.velocity.normalized
              * hit.distance, hit.normal).normalized;
